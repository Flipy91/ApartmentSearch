...
namespace FrankfurtWohnungsSuchApp
{
    public class NassauischHeimCrawler : IApartmentCrawler
    {
        ...
        {
            bool isParsed = double.TryParse(text, NumberStyles.Currency, CultureInfo.GetCultureInfo("de-DE"), out value);

            if(!isParsed)_logger.LogWarning($"{text} could not be parsed");
